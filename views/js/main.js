...
var resizePizzas = function(size) { 
  ...
  // Caching window width value since we don't want to calculate this property repeatedly
  ...
  function changePizzaSizes(size) {
    // store pizza container offsetwidths in an array to batch resizing operation in second loop
    var sizes = [];
    var pizzaElems = document.querySelectorAll(".randomPizzaContainer");


    for (var i = 0; i < pizzaElems.length; i++) {
      var dx = determineDx(pizzaElems[i], size);
      var newwidth = (pizzaElems[i].offsetWidth + dx) + 'px';
      sizes[i] = newwidth;
    }
...
// Array for storing pizza container "movers" outside function once mover element collection
// has been generated by the anon fn in the DOMContentLoaded event handler
var moverItems = [];
...
function updatePositions() {
  ...
  // array for storing calculated new left positions so that style manipulation can happen
  // in a second loop afterward
  newPositions = [];

  for (var i = 0; i < moverItems.length; i++) {
    var phase = Math.sin(fromTop + (i % 5));
    newPositions[i] = moverItems[i].basicLeft + 100 * phase + 'px';
  }

  for (var i = 0; i < moverItems.length; i++) {
    moverItems[i].style.left = newPositions[i];
  }
